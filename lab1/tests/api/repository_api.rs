...
use repo::dto::{repository_dto::ResponseRepoDto, ListResponse};
...
async fn list_all_repos_success() {
    ...
    assert_eq!(
        res.json::<ListResponse<ResponseRepoDto>>(),
        expected_body,
...
async fn create_some_repos(client: &TestServer) {
        ...
        .enumerate()
    {
        ...
        let _ = client.post("/apiV1/repo").json(&create_dto).await;
    }
}